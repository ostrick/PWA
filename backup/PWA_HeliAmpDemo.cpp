...
//----------------------------------------------------------------------
  // write helicity amplitudes of model input
  TComplex H_maid;
  FILE* Hfile; 
  Hfile = fopen("Heli.dat", "w+");
  ...
  for(Int_t n=0; n<Nsteps; n++)
  {
    CosTheta = DCosTheta*n + DCosTheta/2.0 - 1.0; //Calculate central position of current costheta step
      printf("\n %4.4f %4.4f ", gEnergy, CosTheta);
      fprintf(Hfile, "\n %5.4f %5.4f ", gEnergy, CosTheta);
      for(Int_t i=1; i<5; i++)
      {
       H_maid = maid_H(i, CosTheta, eM);
       printf("%10.6f %10.6f ", H_maid.Re(), H_maid.Im());
       fprintf(Hfile, "%10.6f %10.6f ", H_maid.Re(), H_maid.Im());
      }
  }
  }
  fclose(Hfile);
