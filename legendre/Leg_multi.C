...
//program to plot different values of energies for a specific order of legendre polynoms
...
//----------------------------------------------------------------------
  ...
  gr->SetTitle(Form("E_{#gamma}: %.0fMeV",ENERGY[i-1]));
  gr->SetMarkerStyle(6);
...
//---------------------------------------------------------------------- 
  ...
  gMinuit->SetFCN(fcn);  //opens the funtion fcn for chi square calculation
   ...
   for (Int_t l=0; l<=polynumber; l++)
   {
	   gMinuit->GetParameter(l,outpar[l],error[l]);
   }
  
    TF1* fitfunction = new TF1("fitfunction",legendre,0,180,polynumber+1);   
	for(Int_t l=0; l<=polynumber; l++)
	{fitfunction->SetParameter(l,outpar[l]);}		   
	fitfunction->Draw("SAME");
   
    Double_t cs=0;
    
    cs = chisquare(sigma1,cross1,err1,outpar);
    
    TLatex *text = new TLatex(0.30,0.15,Form("red. Chi^{2}: %.3f",cs));
    text->SetNDC(kTRUE);
    text->Draw();
