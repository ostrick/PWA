...
//This program calculates the coefficients for different angular momentum
...
//-----------------------------------------------------------------------------
  ...
  Int_t l=0;     //value of angular momentum
//----------------------------------------------------------------
...
{
   Double_t a=0;
   const Int_t nbins = 30;  //amount of datapoints
...
{   Int_t polynumber=2*l;
  ...
  //main function
//-------------------------------------------------------
void minuit(Int_t l1,Int_t lmin, Int_t lmax)
{
	const Int_t polynumber = 2*l1;
  

	
 FILE* file_prakhov;   
 
 Float_t Energy, Dummy1, Dummy2, Dummy3;
 Float_t sigma, crossc, err;
 Float_t ENERGY[246]={}, SIGMA[7380]={}, CROSS[7380]={}, ERROR[7380]={};
 char Buffer[1024];
...
//graphical output of fits
     ...
     Double_t x[Anz]={};
		...
		for(Int_t i=lmin; i<=lmax; i++) {
			if(polynumber==2*i) {
				picnumber=(i-lmin)*2*(lmax+1); } }
...
{    Int_t lin, lout;
	...
	cout<<"L max:"<<endl;
	cin>>lout;	
     TCanvas* c = new TCanvas("c","L-Coeff",6600,1400);
     c->Divide(2*lout+2,lout-lin+1);	

    for(Int_t i=lin; i<=lout; i++) {
    l=i;
    minuit(l,lin,lout); }
	c->Print("legendre_l.pdf");
}
